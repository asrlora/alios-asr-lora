...
//            esc_tag);
//    safesprintf(printbuf, totallen, offset, tmpbuf);
    printf("%sName               State    Prio StackSize MinFreesize Runtime Candidate\r\n",esc_tag);
    printf("%s------------------------------------------------------------------------\r\n",esc_tag);
...
//                 task->stack_size, free_size, (unsigned long long)time_total, yes);
        printf("%s%-19.18s%-9s%-5d%-10d%-12zu%-9llu%-11c\r\n",esc_tag, task_name, cpu_stat[taskstate], task->prio,
                    task->stack_size, free_size, (unsigned long long)time_total, yes);
...
//                 task->stack_size, free_size, (unsigned int)time_total, yes);
        printf("%s%-19s%-9s%-5d%-10d%-12u%-9u%-11c\r\n",esc_tag, task_name, cpu_stat[taskstate], task->prio,(int)task->stack_size, free_size, (unsigned int)time_total, yes);
...
//                    g_sched_disable_max_time);
    printf("%sMax sched disable time  :%-10d\r\n", esc_tag, g_sched_disable_max_time);
#else
//    plen += sprintf(buf + plen, "%sMax sched disable time  :%-10d\r\n", esc_tag, 0);
     printf("%sMax sched disable time  :%-10d\r\n", esc_tag, 0);
#endif
...
//                    g_intrpt_disable_max_time);
    printf("%sMax intrpt disable time :%-10d\r\n", esc_tag,g_intrpt_disable_max_time);
#else
//    plen += sprintf(buf + plen, "%sMax intrpt disable time :%-10d\r\n", esc_tag, 0);
    printf("%sMax intrpt disable time :%-10d\r\n", esc_tag, 0);
#endif
                      ...
                      char **argv)
{
    ...
    if (argc >= 2  && 0 == strcmp(argv[1], "task")) {
        ...
        printf("%sdumpsys help:\r\n", esc_tag);
        printf("%s\tdumpsys task         : show the task info.\r\n", esc_tag);
//        int len = 0;
...
#ifndef CSP_LINUXHOST
        printf("%s\tdumpsys task_stack   : show the task stack info.\r\n", esc_tag);
//        len += snprintf(pcWriteBuffer + len, xWriteBufferLen - len,
//                        "%s\tdumpsys task_stack   : show the task stack info.\r\n", esc_tag);
#endif
        printf("%s\tdumpsys mm_info      : show the memory has alloced.\r\n", esc_tag);
//        len += snprintf(pcWriteBuffer + len, xWriteBufferLen - len,
//                       "%s\tdumpsys mm_info      : show the memory has alloced.\r\n", esc_tag);
#if (RHINO_CONFIG_MM_LEAKCHECK > 0)
            printf("%s\tdumpsys mm_leak      : show the memory maybe leak.\r\n", esc_tag);
            printf("%s\tdumpsys mm_monitor   : [start/stop] [round time] fire a timer\n to monitor mm, default 10s.\r\n",esc_tag);
//        len += snprintf(pcWriteBuffer + len, xWriteBufferLen - len,
...
#endif
#if (RHINO_CONFIG_CPU_USAGE_STATS > 0)
            printf("%s\tdumpsys info         : show the system info\r\n", esc_tag);
//        len += snprintf(pcWriteBuffer + len, xWriteBufferLen - len,
...
//    memset(printbuf, 0, totallen);
    ...
    rst =  krhino_task_stack_cur_free(task, (size_t*)&offset);
    if (rst == RHINO_SUCCESS) {
...
int dump_task_stack_byname(char *taskname)
{
    ...
    for (tmp = taskhead->next; tmp != taskend; tmp = tmp->next) {
        ...
        if (printall == 1 || strcmp(taskname, task->task_name) == 0) {
              printf("%s------task %s stack -------",esc_tag, task->task_name);
